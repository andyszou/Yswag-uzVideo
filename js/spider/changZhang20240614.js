...
class ChangZhang20240614 extends WebApiBase {
    ...
    async getNewSiteDomain() {
        try {
            const res = await req(this.host, { headers: this.headers })
    ...
    async getVideoList(args) {
        ...
        try {
            ...
            if (proData) {
                var document = parse(proData)
                var allVideo = document.querySelector('.bt_img.mi_ne_kd.mrb').querySelectorAll('ul > li')
                var videos = []
                for (let index = 0; index < allVideo.length; index++) {
                    const element = allVideo[index]
                    var vodUrl = element.querySelector('a')?.attributes['href'] ?? ''
                    var vodPic = element.querySelector('a > img')?.attributes['data-original'] ?? ''
                    var vodName = element.querySelector('a > img')?.attributes['alt'] ?? ''
                    var vodDiJiJi = element.querySelector('div.jidi > span')?.text
                    var vodHD = element.querySelector('div.hdinfo > span.qb')?.text ?? element.querySelector('div.hdinfo > span.furk')?.text
    ...
    async getVideoDetail(args) {
        ...
        try {
            ...
            if (proData) {
                var document = parse(proData)
                var vod_pic = document.querySelector('.dyimg.fl > img')?.attributes['src'] ?? ''
                var vod_name = document.querySelector('div.moviedteail_tt > h1')?.text ?? ''
                var detList = document.querySelector('.moviedteail_list')?.querySelectorAll('li') ?? []
                var vod_year = ''
                var vod_director = ''
                var vod_actor = ''
                var vod_area = ''
                ...
                var juJiDocment = document.querySelector('.paly_list_btn')?.querySelectorAll('a') ?? []
    ...
    async getVideoPlayUrl(args) {
        ...
        try {
            ...
            if (proData) {
                    ...
                    if (pro2.data) {
                        ...
                        if (scripts.length - 2 > 0) {
                            ...
                            if (code1.indexOf('var player') > -1) {
                                ...
                                // console.log(rand[1]);
                                let content = JSON.parse(this.cryptJs(player[1], 'VFBTzdujpR9FWBhe', rand[1]))
                                ...
                                let playUrl = temp.substring(0x0, (temp.length - 0x7) / 0x2) + temp.substring((temp.length - 0x7) / 0x2 + 0x7)
                ...
                let x = document.querySelectorAll('script:contains(window.wp_nonce)') ?? []
                if (x.length > 0) {
                    let code = x[0].text
